...
// because when the index of something that's missing is -1
...
let colors = ["Blue", "Yellow", "White", "Black", "Green"];
for (let i = 0; i < colors.length; i++) {
  console.log(`My ${i + 1}# choise is ${colors[i]}`);
}

let suffixArr = ["st", "nd", "rd", "th"];
for (let i = 0; i < colors.length; i++) {
  if (i < 3) console.log(`My ${i + 1}${suffixArr[i]} choise is ${colors[i]}`);
  else {
    console.log(`My ${i + 1}${suffixArr[3]} choise is ${colors[i]}`);
  }
}
...
if (
  ...
  building["numberOfRoomsAndRent"]["dan"][1] +=
    1200 - building["numberOfRoomsAndRent"]["dan"][1];
}
